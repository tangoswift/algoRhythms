...
function testGetWidgetUrl_notSpecified_withQueryAndFragment() {
  ...
  assertEquals(firebaseui.auth.util.getCurrentUrl(), widgetUrl)
  // Only the mode query param should be overwritten.
  widgetUrl = config.getWidgetUrl(
    firebaseui.auth.widget.Config.WidgetMode.SELECT
  )
  assertEquals(
    'http://www.example.com/path/?mode2=bar&mode=select#a=1',
    widgetUrl
  )
  ...
  assertEquals(
    'http://www.example.com/path/?mode=foo&mode2=select#a=1',
    widgetUrl
  )
}
...
function testGetRecaptchaParameters() {
  ...
  config.update('signInOptions', [
    'github.com',
    {provider: 'google.com'},
    {provider: 'phone', recaptchaParameters: blacklist},
    'password'
  ])
  assertObjectEquals({}, config.getRecaptchaParameters())
...
function testGetGoogleYoloConfig_notEnabled() {
  ...
  config.update('credentialHelper', firebaseui.auth.CredentialHelper.NONE)
...
function testUpdateConfig_phoneSignInOption_error() {
  ...
  assertEquals('WhitelistedCountries must be a non-empty array.', error.message)
...
function testGetCallbacks() {
  ...
  var signInSuccessCallback = function() {
    return true
  }
  var signInSuccessWithAuthResultCallback = function() {
    return true
  }
...
function testGetCredentialHelper_httpOrHttps() {
  ...
  stub.replace(firebaseui.auth.util, 'isHttpOrHttps', function() {
    return true
  })
