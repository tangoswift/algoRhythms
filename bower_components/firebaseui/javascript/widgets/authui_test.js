...
function setUp() {
  ...
  for (var key in firebaseui.auth.idp.AuthProviders) {
    ...
    if (key == 'password') {
      ...
      firebase.auth.EmailAuthProvider.EMAIL_PASSWORD_SIGN_IN_METHOD = 'password'
...
function testStart() {
  ...
  assertFalse(firebaseui.auth.storage.hasPendingRedirectStatus(app1.getAppId()))
  ...
  assertFalse(firebaseui.auth.storage.hasPendingRedirectStatus(app2.getAppId()))
  app1.getAuth().assertSignOut([])
...
function testStartSignInWithCredential_error() {
  ...
  app.getExternalAuth().install()
  asyncTestCase.waitForSignals(1)
  app.startSignInWithCredential(expectedCredential).then(fail, function(error) {
    assertEquals(expectedError, error)
    asyncTestCase.signal()
  })
  app
...
function testStartSignInWithRedirect_upgradeAnonymous_error() {
  ...
  app
    .getExternalAuth()
    .currentUser.assertLinkWithRedirect([expectedProvider], null, expectedError)
  app.getAuth().process()
  app.getExternalAuth().process()
}
...
function testFinishSignInAndRetrieveDataWithAuthResult_success() {
  ...
  app
    ...
    .then(function() {
      app.getExternalAuth().assertUpdateCurrentUser([expectedUser], function() {
        app.getExternalAuth().setUser(expectedUser)
