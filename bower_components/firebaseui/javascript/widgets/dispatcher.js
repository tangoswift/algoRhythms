 ...
 * @private
 */
firebaseui.auth.widget.dispatcher.ELEMENT_NOT_FOUND_ =
 ...
 * @private
 */
firebaseui.auth.widget.dispatcher.getRequiredUrlParam_ = function(
  paramName,
  opt_url
) {
  return goog.asserts.assertString(
    goog.uri.utils.getParamValue(
      opt_url || firebaseui.auth.util.getCurrentUrl(),
      paramName
    )
  )
}
...
firebaseui.auth.widget.dispatcher.getActionCode_ = function(opt_url) {
  return firebaseui.auth.widget.dispatcher.getRequiredUrlParam_(
    'oobCode',
    opt_url
  )
}
