  ...
  if (!this.getRedirectResult_) {
    ...
    var cb = function(user) {
      if (
        ...
        // some reason.
        return /** @type {!goog.Promise<!firebase.auth.UserCredential>} */ (goog.Promise.resolve(
          self
            .getExternalAuth()
            .getRedirectResult()
            .then(
...
firebaseui.auth.AuthUI.prototype.isPendingRedirect = function() {
  ...
  this.checkIfDestroyed_()
  return (
    firebaseui.auth.storage.hasPendingRedirectStatus(this.getAppId()) ||
...
firebaseui.auth.AuthUI.prototype.reset = function() {
  ...
  this.getRedirectResult_ = /** @type {!goog.Promise<!firebase.auth.UserCredential>} */ (goog.Promise.resolve(
    {
      user: null,
      credential: null
    }
...
firebaseui.auth.AuthUI.prototype.initPageChangeListener_ = function(element) {
  ...
  goog.events.listen(this.widgetEventDispatcher_, 'pageEnter', function(event) {
    // Get new page ID.
...
firebaseui.auth.AuthUI.prototype.showOneTapSignIn = function(handler) {
  ...
  try {
    this.googleYolo_
      .show(this.getConfig().getGoogleYoloConfig(), this.isAutoSignInDisabled())
...
firebaseui.auth.AuthUI.prototype.sendSignInLinkToEmail = function(
  ...
  var actionCodeSettings = /** @type {!firebase.auth.ActionCodeSettings} */ (this.getConfig().getEmailLinkSignInActionCodeSettings())
  ...
  var cb = function(user) {
    ...
    return /** @type {!firebase.Promise<void>} */ (self
      .getAuth()
...
firebaseui.auth.AuthUI.prototype.startSignInWithEmailAndPassword = function(
  ...
  return /** @type {!firebase.Promise<!firebase.auth.UserCredential>} */ (this.getAuth()
 ...
 * @return {!firebase.Promise<!firebase.auth.UserCredential>}
 */
firebaseui.auth.AuthUI.prototype.startCreateUserWithEmailAndPassword = function(
  email,
  password
) {
  // Check if instance is already destroyed.
  ...
  var cb = function(user) {
    if (user) {
      ...
      return /** @type {!firebase.Promise<!firebase.auth.UserCredential>} */ (user.linkWithCredential(
        credential
      ))
    } else {
      ...
      return /** @type {!firebase.Promise<!firebase.auth.UserCredential>} */ (self
        .getAuth()
...
firebaseui.auth.AuthUI.prototype.startSignInWithCredential = function(
  ...
  var cb = function(user) {
    if (user) {
      ...
      // Auth user. Otherwise merge conflict will always occur when linking the
      // credential to the external anonymous user after creation.
      return /** @type {!firebase.Promise<!firebase.auth.UserCredential>} */ (user
 ...
 */
firebaseui.auth.AuthUI.prototype.finishSignInAndRetrieveDataWithAuthResult = function(
  authResult
) {
  // Check if instance is already destroyed.
  this.checkIfDestroyed_()
  ...
  var cb = function(user) {
    if (
      ...
      return /** @type {!firebase.Promise<!firebaseui.auth.AuthResult>} */ (self
        .clearTempAuthState()
        .then(function() {
 ...
 * @return {!firebase.Promise<!firebase.auth.UserCredential>}
 */
firebaseui.auth.AuthUI.prototype.signInWithExistingEmailAndPasswordForLinking = function(
  email,
  password
) {
  // Check if instance is already destroyed.
