...
function setUp() {
  ...
  stub.replace(firebaseui.auth.AuthUI, 'getAuthUi', function() {
    return app
  })
...
function assertSelectFromAccountChooserInvoked(
  ...
  var selectFromAccountChooser =
    firebaseui.auth.widget.handler.common.selectFromAccountChooser
  assertEquals(1, selectFromAccountChooser.getCallCount())
  assertEquals(app, selectFromAccountChooser.getLastCall().getArgument(0)())
  assertEquals(container, selectFromAccountChooser.getLastCall().getArgument(1))
  assertEquals(
    opt_disableSelectOnEmpty,
...
function assertHandlerInvoked(handlerName, var_args) {
  var handler = firebaseui.auth.widget.handlers_[handlerName]
...
function testGetRedirectUrl() {
  ...
  var url = 'http://localhost/callback?mode=select'
  assertEquals(null, firebaseui.auth.widget.dispatcher.getRedirectUrl(app, url))
  // Set current page URL to include a redirect URL.
  url =
    'http://localhost/callback?mode=select&signInSuccessUrl=' +
  ...
  // Update the query parameter for redirect URL.
  url =
    'http://localhost/callback?mode=select&continue=' +
    encodeURIComponent(redirectUrl)
...
function setModeAndUrlParams(mode, opt_params) {
  ...
  stub.replace(firebaseui.auth.util, 'getCurrentUrl', function() {
...
function testDispatchOperation_noStorageSupport_elementNotFound() {
  ...
  stub.set(firebaseui.auth.storage, 'isAvailable', function() {
    return false
  })
...
function testDispatchOperation_revokeChangeEmail() {
  var element = goog.dom.createElement('div')
  setModeAndUrlParams(firebaseui.auth.widget.Config.WidgetMode.RECOVER_EMAIL, {
    oobCode: 'ACTION_CODE'
  })
  firebaseui.auth.widget.dispatcher.dispatchOperation(app, element)
  assertHandlerInvoked(
...
function testDispatchOperation_verifyEmail() {
  var element = goog.dom.createElement('div')
  setModeAndUrlParams(firebaseui.auth.widget.Config.WidgetMode.VERIFY_EMAIL, {
    oobCode: 'ACTION_CODE'
  })
  firebaseui.auth.widget.dispatcher.dispatchOperation(app, element)
  assertHandlerInvoked(
...
function testDispatchOperation_emailLinkSignIn() {
  var element = goog.dom.createElement('div')
  setModeAndUrlParams(firebaseui.auth.widget.Config.WidgetMode.SIGN_IN, {
    oobCode: 'ACTION_CODE',
    lang: 'en'
  })
  firebaseui.auth.widget.dispatcher.dispatchOperation(app, element)
...
function testDispatchOperation_resetPassword() {
  var element = goog.dom.createElement('div')
  setModeAndUrlParams(firebaseui.auth.widget.Config.WidgetMode.RESET_PASSWORD, {
    oobCode: 'ACTION_CODE'
  })
  firebaseui.auth.widget.dispatcher.dispatchOperation(app, element)
  assertHandlerInvoked(
