...
function testSetLoggedInWithAuthResult() {
  ...
  return testAuth
    ...
    .then(function() {
      ...
      assertEquals(
        1,
        firebaseui.auth.storage.getRememberedAccounts(app.getAppId()).length
      )
      assertObjectEquals(
        passwordAccount,
...
function testHandleSignInFetchSignInMethodsForEmail_disabledFederatedAcct() {
  ...
  assertFalse(firebaseui.auth.storage.hasPendingEmailCredential(app.getAppId()))
...
function testLoadAccountchooserJs_notLoaded() {
  ...
  testStubs.replace(goog.net.jsloader, 'safeLoad', function() {
...
function testLoadAccountchooserJs_notSupported() {
  ...
  testStubs.replace(firebaseui.auth.sni, 'isSupported', function() {
    return false
  })
  accountchooser = undefined
  firebaseui.auth.widget.handler.common.acLoader_ = null
...
function testGetErrorMessage_unknownError_jsonMessage() {
  testStubs.replace(firebaseui.auth.log, 'error', goog.testing.recordFunction())
  var backendMessage = {
    error: {
      errors: [
        {
  ...
  assertEquals(firebaseui.auth.soy2.strings.internalError().toString(), message)
}
...
function testIsPasswordProviderOnly_multipleMixedProviders() {
  ...
  assertFalse(firebaseui.auth.widget.handler.common.isPasswordProviderOnly(app))
}
...
function testHandleGoogleYoloCredential_unhandled_withoutScopes() {
  ...
  testAuth.assertSignInWithCredential([expectedCredential], null, internalError)
  testAuth.process().then(function() {
    // Remains on the same page.
    assertProviderSignInPage()
    // Confirm error message shown in info bar.
