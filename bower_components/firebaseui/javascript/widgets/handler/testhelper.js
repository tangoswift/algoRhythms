...
function setUp() {
  ...
  firebase['auth']['SAMLAuthProvider'].prototype.setCustomParameters = function(
    customParameters
  ) {
    this.customParameters = customParameters
    return this
...
function getPhoneCountrySelectorElement() {
  return goog.dom.getElementByClass('firebaseui-id-country-selector', container)
}
...
function getRecaptchaElement() {
  return goog.dom.getElementByClass('firebaseui-recaptcha-container', container)
}
...
function getKeysForCountrySelectorButtons() {
  var buttons = goog.dom.getElementsByClass('firebaseui-list-box-dialog-button')
...
function assertDialog(message) {
  ...
  assertTrue(goog.dom.classlist.contains(dialogElement, 'firebaseui-id-dialog'))
...
function assertSignInFailure(expectedError) {
  ...
  assertObjectEquals(
    expectedError,
    signInFailureCallback.getLastCall().getArgument(0)
  )
