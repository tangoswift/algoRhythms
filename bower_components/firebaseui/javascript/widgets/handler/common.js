...
firebaseui.auth.widget.handler.common.loadAccountchooserJs = function(
  ...
  firebaseui.auth.widget.handler.common.acForceUiShown_ = !!opt_forceUiShownCallback
  ...
  if (!firebaseui.auth.widget.handler.common.acLoader_) {
    if (
      ...
      firebaseui.auth.widget.handler.common.acLoader_ = goog.Promise.resolve(
        goog.net.jsloader.safeLoad(src)
      ).thenCatch(function() {})
    } else {
 ...
 * @private
 */
firebaseui.auth.widget.handler.common.handleAcAccountSelectedResponse_ = function(
  account,
  app,
  container,
  uiShownCallback
  ...
  var continueCallback = function() {
    ...
    var isPasswordProviderOnly = firebaseui.auth.widget.handler.common.isPasswordProviderOnly(
      app
    )
    app.registerPending(
      app
        .getAuth()
        .fetchSignInMethodsForEmail(account.getEmail())
 ...
 * @private
 */
firebaseui.auth.widget.handler.common.handleAcAddAccountResponse_ = function(
  isAvailable,
  app,
  container,
  uiShownCallback
...
firebaseui.auth.widget.handler.common.setLoggedInWithAuthResult = function(
  ...
  var onError = function(error) {
    ...
    if (firebaseui.auth.widget.handler.common.isCredentialExpired(error)) {
      ...
      if (error['code']) {
        ...
        errorMessage = firebaseui.auth.widget.handler.common.getErrorMessage(
          error
        )
      }
  ...
  // default.
  if (
    !firebaseui.auth.storage.hasRememberAccount(app.getAppId()) ||
 ...
 * @private
 */
firebaseui.auth.widget.handler.common.setUserLoggedInExternalWithAuthResult_ = function(
  app,
  component,
  authResult
) {
...
firebaseui.auth.widget.handler.common.getAuthProvider_ = function(
  ...
  var additionalScopes = app.getConfig().getProviderAdditionalScopes(providerId)
...
firebaseui.auth.widget.handler.common.handleGoogleYoloCredential = function(
  ...
  var providerId = app
    .getConfig()
    .getProviderIdFromAuthMethod((credential && credential.authMethod) || null)
...
firebaseui.auth.widget.handler.common.verifyPassword = function(
  ...
  app.registerPending(
    component.executePromiseRequest(
      /** @type {function (): !goog.Promise} */ (goog.bind(
        app.startSignInWithEmailAndPassword,
        app
      )),
      [email, password],
      function(userCredential) {
...
firebaseui.auth.widget.handler.common.handleStartEmailFirstFlow = function(
  ...
  app.registerPending(
    component.executePromiseRequest(
      /** @type {function (): !goog.Promise} */ (goog.bind(
        app.getAuth().fetchSignInMethodsForEmail,
        app.getAuth()
      )),
      [email],
 ...
 */
firebaseui.auth.widget.handler.common.handleSignInFetchSignInMethodsForEmail = function(
  app,
  container,
  signInMethods,
  email,
  ...
  if (!signInMethods.length && app.getConfig().isEmailPasswordSignInAllowed()) {
    ...
    // a different email.
    var federatedSignInMethod = firebaseui.auth.idp.getFirstFederatedSignInMethod(
      signInMethods,
        ...
        if (firebaseui.auth.acClient.isInitialized()) {
          ...
          var continueCallback = function() {
            ...
            firebaseui.auth.acClient.trySelectAccount(
              ...
              app
                .getConfig()
                .getWidgetUrl(firebaseui.auth.widget.Config.WidgetMode.SELECT)
            )
          }
