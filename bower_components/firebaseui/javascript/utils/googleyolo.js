...
firebaseui.auth.GoogleYolo = function(opt_googleyolo) {
   ...
   */
  this.googleyolo_ =
    opt_googleyolo || goog.global[firebaseui.auth.GoogleYolo.NAMESPACE_]
...
firebaseui.auth.GoogleYolo.prototype.cancel = function() {
  ...
  if (this.googleyolo_ && this.initialized_) {
    this.lastCancel_ = this.googleyolo_
      .cancelLastOperation()
 ...
 *     the process.
 */
firebaseui.auth.GoogleYolo.prototype.show = function(
  config,
  autoSignInDisabled
) {
  var self = this
  ...
  if (this.googleyolo_ && config) {
    ...
    var render = function() {
      ...
      if (!autoSignInDisabled) {
        retrieveCredential = self.googleyolo_
          .retrieve(/** @type {!SmartLockRequestOptions} */ (config))
          .catch(function(error) {
            // For user cancellation or concurrent request pass down.
            // Otherwise suppress and run hint.
            if (
              error.type === firebaseui.auth.GoogleYolo.Error.USER_CANCELED ||
              error.type === firebaseui.auth.GoogleYolo.Error.CONCURRENT_REQUEST
            ) {
              throw error
            }
  ...
  if (!goog.global[firebaseui.auth.GoogleYolo.NAMESPACE_]) {
    ...
    this.loader_ = firebaseui.auth.util.onDomReady().then(function() {
      ...
      return new goog.Promise(function(resolve, reject) {
        ...
        goog.Promise.resolve(goog.net.jsloader.safeLoad(url)).thenCatch(
          function(error) {
