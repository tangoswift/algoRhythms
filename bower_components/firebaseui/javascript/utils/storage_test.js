...
function initializeCookieStorageMock(maxAge, path, domain, secure) {
  ...
  mockCookieStorage = {}
  stubs.replace(goog.net.cookies, 'set', function(
    key,
    value,
    actualMaxAge,
    actualPath,
  ...
  stubs.replace(goog.net.cookies, 'remove', function(
    key,
    actualPath,
    actualDomain
  ) {
...
function testRememberAccountAndGetRemoveRememberedAccounts() {
  ...
  var account4 = new firebaseui.auth.Account(
    'user2@example.com',
    'New Test User2'
  )
...
function testGetSetRemoveEncryptedPendingCredential_withAppId() {
  ...
  assertNull(firebaseui.auth.storage.getEncryptedPendingCredential(key2, appId))
  assertObjectEquals(
    pendingEmailCred2,
